...
$commentUser = (isset($_SESSION['username']))? $_SESSION['username'] : 'Anonymous';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $conn->prepare("INSERT INTO comment (context, creation_date, username, articleId) VALUES (?, NOW(), ?, ?)");
    $stmt->execute([$_POST['Message'], $commentUser, $id]);
}

$comments = $conn->query("SELECT * FROM comment WHERE articleId = $id ORDER BY creation_date DESC")->fetchAll();
?>
<!DOCTYPE html>
                    ...
                    <div class="single-post-body">
                        ...
                        <div class="single-blog-inner">
                            ...
                            <div class="post-coment-area">
                                <h4 class="coments-area-title"><?php echo count($comments); ?> Comments</h4>

                                <?php if (count($comments) > 0): ?>
                                    <?php foreach ($comments as $comment): ?>
                                        <div class="post-comment-item">
                                            <div class="post-comment-author">
                                                <img src="images/comment1.png" alt="" class="post-author-image" />
                                            </div>
                                            <div class="post-comment-description">
                                                <div class="upper-description">
                                                    ...
                                                    <div class="post-author-time">
                                                        - <?php echo date('M d, Y', strtotime($comment['creation_date'])); ?>
                                                    </div>
                                                </div>
                            ...
                            <div class="comment-form w-form">
                                <form id="email-form2" method="POST" action="">
                                    <div class="form-inputs">
                                            <p>Commenting as: <strong><?php echo $commentUser ?></strong></p>

                                        <textarea name="Message" rows="5" id="Message" placeholder="Write your message here..." class="input-style input-style-single textarea w-input" required></textarea>

                                        <input type="submit" value="Submit" class="form-button form-button2 w-button" />
                                    </div>
                                </form>
                            </div>
                        </div>
